...
$(document).ready(function() {
    ...
    $(".capture_button").on('click', function() {
        ...
        if ($("#capture_button").attr('disabled') != "disabled")
...
function AcceptPictureFromUser(picture) {
    ...
    $("#send_button").css("background-image", "url(assets/image/send_button.png)");
    $("#send_button").on("click", function() {
        $(this).css("background-image", send_image_active_preload.src);
    });

    $("#capture_button").attr("disabled", "disabled");
    $("#capture_button").css("background-image", "url(assets/image/capture_button_inactive.png)");
}
...
function PushImage(blob) {
    ...
    alert("Pushing for duration: " + dur);
    var identities = [];
    var image = [];
    image.push(dur);
